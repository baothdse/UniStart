...
public interface ReviewRepository extends JpaRepository<Review, Integer> {
	...
	@Query("select r.starCare from Review r where r.university.id = ?1 and r.isActive = true")
	List<Integer> getStarCare(int universityId);
	@Query("select r.starFacilities from Review r where r.university.id = ?1 and r.isActive = true")
	List<Integer> getStarFacilities(int universityId);
	@Query("select r.starSocieties from Review r where r.university.id = ?1 and r.isActive = true")
	List<Integer> getStarSocieties(int universityId);
	@Query("select r.starTeaching from Review r where r.university.id = ?1 and r.isActive = true")
	List<Integer> getStarTeaching(int universityId);
	@Query("select r.starCareer from Review r where r.university.id = ?1 and r.isActive = true")
	List<Integer> getStarCareer(int universityId);
	
	@Query("select count (r) from Review r where r.isRecomment = true and r.university.id = ?1 and r.isActive = true")
